...
class JdbcTemplateTicketRepository(private val databaseTicketRepository: JdbcTemplate): TicketRepository {
    ...
    override fun get(id: UUID): Ticket {
        ...
        val ticketResult: List<Ticket> = databaseTicketRepository.query(
            "SELECT id, ticket_number, owner_email, owner_phone_number, owner_email, status, shoe_description, completion_date where id=?",
             {
                it: ResultSet, _: Int ->
                    Ticket(
                        ...
                        completionDate = OffsetDateTime.of(
